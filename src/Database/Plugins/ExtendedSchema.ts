...
function ExtendedSchemaPlugin(schema: Schema, options: any) {
    ...
    schema.virtual('wasNew')
        ...
        .set(function(value: any) { this._wasNew = value })

    schema.virtual('wasModified')
        .get(function() { return this._wasModified })
        .set(function(value: any) { this._wasModified = value })
    ...
    schema.post('save', function(doc: ExtendedDocument) {
        if (doc.wasNew) schema.emit('OnCreated', doc)
        if (!doc.wasNew && doc.wasModified) schema.emit('OnUpdated', doc)
    })

    schema.post('remove', function(doc: ExtendedDocument) {
        schema.emit('OnDeleted', doc)
    })
