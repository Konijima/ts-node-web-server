...
import { ExtendedUserDocument, ExtendedUserSchemaPlugin } from '@Database/Plugins/ExtendedUserSchema'
import { ISessionDocument, Session } from '@Models/Session'
...
interface IUserModel extends Model<IUserDocument> {
     ...
     */
    Login: (username: string, password: string) => { user: IUserDocument, session: ISessionDocument }
     ...
     */
    Register: (username: string, email: string, password: string, createSession?: boolean) => { user: IUserDocument, session: ISessionDocument }
...
const UserSchema: Schema<IUserDocument> = new Schema({
    ...
    username: {
        ...
        maxlength: [SETTINGS.USERNAME_LENGTH.MAX, 'username_too_long'],
    ...
    email: {
        ...
        match: [/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/, 'email_invalid_format'],
    },
...
UserSchema.static('Login', async function (this: IUserModel, username: string, password: string) {
    let user = await this.findOne({ username }).select('+email +password')
    if (!user) await this.findOne({ email: username }).select('+email +password')
    if (!user) throw new Error('user_not_found')

    if (await user.comparePassword(password) === false) throw new Error('invalid_credentials')
...
UserSchema.static('Register', async function (this: IUserModel, username: string, email: string, password: string, createSession: boolean = false) {
